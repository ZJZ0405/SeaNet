...
class InvertedResidual(nn.Module):
    def __init__(
        ...
        if expand_ratio != 1:
            # pw
            layers.append(
                Conv2dNormActivation(inp, hidden_dim, kernel_size=1, norm_layer=norm_layer, activation_layer=nn.ReLU6, dilation=dilation)
            )
        layers.extend(
            [
                # dw
...
def mobilenet_v2(pretrained=True, progress=True, **kwargs) -> MobileNetV2:
